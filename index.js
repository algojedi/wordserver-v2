...
app.get("/define", async (req, res) => {
	...
	try {
		...
		//console.error(error);
		console.log(
			"error in api lookup: likely caused by non-word input"
		);
		res.status(400).send();
	}
});
...
app.post("/addWordToCart", async (req, res) => {
	...
	try {
		...
		if (!user) {
			...
			return res
				.status(400)
				.json("unable to find user in addWord route");
		}
...
app.post("/removeWord", async (req, res) => {
	...
	try {
		...
		if (!user) {
			return res
				.status(400)
				.json("unable to find user profile on delete");
		}
	...
	try {
		...
		if (!user) {
			return res
				.status(400)
				.json(
					"unable to find user profile on delete all words"
				);
		}
		user.emptyCart();
